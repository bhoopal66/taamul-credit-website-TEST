...
import { validateCallback, sanitize } from "../_shared/validation";
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const validation = validateCallback(data);
    if (!validation.valid) {
    ...
    const payload = {
      ...
      preferredTime: sanitize(data.preferredTime),
    };
    ...
    if (!result.success) {
      return jsonResponse({ success: false, error: "Failed to save request" }, 500, request, env);
    }
